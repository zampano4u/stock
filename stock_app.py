...
# ✅ 종목 선택 UI
selected_ticker = st.selectbox("🔍 분석할 종목을 선택하세요", st.session_state.tickers)
...
try:
    ...
    if ath:
        ...
        ax.set_ylabel("가격 ($)")
        ax.set_title(f"{selected_ticker} 현재가 위치", fontsize=10)
        st.pyplot(fig)
    ...
    if not hist.empty:
        ...
        ax2.set_ylabel("가격 ($)")
        ax2.set_xlabel("날짜")
        ax2.set_title(f"{selected_ticker} - 최근 1년간 주가 추세", fontsize=10)
        ax2.legend()
        ax2.grid(True)
        st.pyplot(fig2)

except Exception as e:
    st.error(f"{selected_ticker} 조회 실패: {e}")
